...
export const FeaturedServices: React.FC = () => {
  const { setPage, setSelectedService, addToCart } = useStore();
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between mb-10">
          <div>
            ...
            <p className="text-bridge-gray text-sm mt-1">Top-rated services from verified sellers</p>
          </div>
          <button
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 md:gap-6">
          {featuredServices.map((service) => {
            ...
            return (
              <div
                key={service.id}
                className="group relative flex flex-col sm:flex-row gap-0 sm:gap-0 overflow-hidden rounded-2xl bg-bridge-dark-2/60 border border-white/[0.06] hover:border-white/15 transition-all duration-500 card-float"
              >
                {/* Image Side */}
                <div className="relative w-full sm:w-[45%] lg:w-[42%] flex-shrink-0 overflow-hidden">
                  ...
                  <div className="absolute inset-0 bg-gradient-to-r from-transparent via-transparent to-bridge-dark-2/60 hidden sm:block"></div>
                  <div className="absolute inset-0 bg-gradient-to-t from-bridge-dark-2/80 via-transparent to-transparent sm:hidden"></div>
                  ...
                  <div className="absolute top-3 right-3 px-2.5 py-1 glass-strong text-white text-[10px] font-semibold rounded-lg">
                    {cat?.icon} {cat?.name}
                  </div>
                </div>
                ...
                <div className="flex-1 flex flex-col p-4 sm:p-5">
                  ...
                  >
                    <h3 className="text-base md:text-lg font-bold text-white group-hover:text-bridge-primary-light transition-colors line-clamp-2 leading-snug">
                      {service.title}
                    </h3>
                  </button>

                  {/* Description */}
                  <p className="text-xs text-bridge-gray line-clamp-2 mb-3 leading-relaxed">{service.description}</p>

                  {/* Features Tags */}
                  <div className="flex flex-wrap gap-1.5 mb-4">
                    {service.features.slice(0, 3).map((f, fi) => (
                      <span key={fi} className="px-2 py-0.5 text-[10px] font-medium bg-white/5 text-bridge-gray rounded-md border border-white/5">
                        {f}
                      </span>
                    ))}
                  ...
                  <div className="flex items-center justify-between pt-3 border-t border-white/[0.06]">
                    <div className="flex items-center gap-3">
                      <div className="flex items-center gap-1">
                        ...
                        <span className="text-[10px] text-bridge-gray">({service.reviewCount})</span>
                      </div>
                    ...
                    <div className="flex items-center gap-3">
                      <div className="text-right">
                        <p className="text-[10px] text-bridge-gray">From</p>
                        <p className="text-lg font-black text-white leading-none">৳{service.price.toLocaleString()}</p>
                      </div>
                      <button
        ...
        <div className="text-center mt-10">
          <button
            onClick={() => setPage('categories')}
            className="inline-flex items-center gap-2 px-8 py-3 glass border border-white/10 hover:border-bridge-primary/30 text-white font-semibold rounded-xl hover:bg-bridge-primary/10 transition-all cursor-pointer"
          >
