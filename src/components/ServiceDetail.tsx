...
export const ServiceDetail: React.FC = () => {
  ...
  const handleContactSeller = () => {
    setSelectedSeller(service.sellerId);
    setPage('messages');
  };

  const shareUrl = `bridge.app/service/${service.id}`;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            ...
            <div className="relative rounded-2xl overflow-hidden mb-6 watermark-overlay">
              ...
              />
              <div className="absolute inset-0 bg-gradient-to-t from-bridge-dark via-transparent to-transparent"></div>
              <div className="absolute bottom-4 left-4 right-4 flex justify-between items-end">
                <div className="px-3 py-1.5 glass-strong rounded-lg text-xs text-white">
                  {cat?.icon} {cat?.name}
                </div>
                <div className="flex items-center gap-1 px-3 py-1.5 glass-strong rounded-lg text-xs text-white">
            ...
            {/* Title */}
            <h1 className="text-2xl md:text-3xl font-bold text-white mb-4">{service.title}</h1>
            ...
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <button 
                  onClick={() => { setSelectedSeller(service.sellerId); setPage('seller-profile'); }}
                  className="text-sm font-semibold text-white hover:text-bridge-primary-light transition-colors flex items-center gap-1 cursor-pointer"
                >
                  {service.sellerName}
            ...
            {/* Tabs */}
            <div className="flex gap-1 p-1 bg-bridge-dark-2 rounded-xl mb-6">
            ...
            {activeTab === 'overview' && (
              <div className="space-y-6">
                <div className="glass rounded-2xl p-6">
                  ...
                  <p className="text-bridge-gray leading-relaxed">{service.description}</p>
                </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {service.features.map((feature, i) => (
                      <div key={i} className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-bridge-gray">{feature}</span>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            {activeTab === 'gallery' && (
              <div className="glass rounded-2xl p-6">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h3 className="text-lg font-bold text-white">Demo Gallery</h3>
                ...
                <div className="grid grid-cols-2 gap-4">
                  {[1,2,3,4].map(i => (
                    <div key={i} className="relative aspect-video bg-bridge-dark-3 rounded-xl overflow-hidden watermark-overlay no-select">
                      <img 
                        src={service.thumbnail} 
                        alt="Demo" 
                ...
                <p className="text-xs text-bridge-gray mt-3 text-center">
                  ...
                  <div key={review.id} className="glass rounded-2xl p-5">
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-bridge-gray">{review.date}</p>
                      </div>
                    ...
                    <p className="text-sm text-bridge-gray">{review.comment}</p>
                  </div>
                ))}
              </div>
            )}
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-24 space-y-4">
              {/* Pricing Card */}
              <div className="glass rounded-2xl p-6 border border-white/10 animate-pulse-glow">
                <div className="text-center mb-6">
                  <span className="text-sm text-bridge-gray">Starting at</span>
                  <div className="text-4xl font-black text-white mt-1">
                    ৳{service.price.toLocaleString()}
                  </div>
                  <span className="text-sm text-bridge-gray">{service.currency}</span>
                </div>

                <div className="space-y-3 mb-6">
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-bridge-gray flex items-center gap-2"><Clock className="w-4 h-4" /> Delivery Time</span>
                    <span className="text-white font-medium">{service.deliveryTime}</span>
                  </div>
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-bridge-gray flex items-center gap-2"><Star className="w-4 h-4" /> Rating</span>
                    <span className="text-white font-medium">{service.rating}/5.0</span>
                  </div>
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-bridge-gray flex items-center gap-2"><Shield className="w-4 h-4" /> Guarantee</span>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button 
                    ...
                    className="py-2.5 glass border border-white/10 text-white text-sm font-medium rounded-xl hover:bg-white/10 transition-all flex items-center justify-center gap-1 cursor-pointer"
                  >
                    <MessageCircle className="w-4 h-4" />
                    Chat
                  </button>
                  <button className="py-2.5 glass border border-white/10 text-white text-sm font-medium rounded-xl hover:bg-white/10 transition-all flex items-center justify-center gap-1 cursor-pointer">
              ...
              <div className="glass rounded-2xl p-5">
                <h4 className="text-sm font-bold text-white mb-3">Share This Service</h4>
                <div className="flex items-center gap-2">
                  <input
                    type="text"
                    value={shareUrl}
                    readOnly
                    className="flex-1 px-3 py-2 bg-bridge-dark-3 border border-white/10 rounded-lg text-xs text-bridge-gray"
                  />
                  <button 
                    onClick={() => {
                ...
                <p className="text-xs text-bridge-gray mt-2">
              ...
              <div className="glass rounded-2xl p-5">
                ...
                <ul className="space-y-2">
                  {['Secure payment processing', 'Money-back guarantee', 'Anti-piracy protection', '24/7 customer support'].map((item, i) => (
                    <li key={i} className="flex items-center gap-2 text-xs text-bridge-gray">
