    ...
    <section className="py-16 md:py-24 relative overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-14">
          ...
          <h2 className="text-2xl md:text-4xl font-black font-display text-white mb-3">
            How <span className="gradient-text">Bridge</span> Works
          </h2>
          <p className="text-bridge-gray text-sm max-w-lg mx-auto">
              ...
              return (
                <div key={i} className="relative md:flex items-center">
                  ...
                  <div className="hidden md:flex absolute left-1/2 -translate-x-1/2 z-10">
                    <div
                      className="w-10 h-10 rounded-full flex items-center justify-center text-white text-xs font-black shadow-lg"
                  ...
                  <div className={`md:w-1/2 ${isLeft ? 'md:pr-12 md:text-right' : 'md:pl-12 md:ml-auto'}`}>
                    <div className="glass-card rounded-2xl p-5 md:p-6 card-hover">
                      <div className={`flex items-center gap-3 mb-3 ${isLeft ? 'md:flex-row-reverse' : ''}`}>
                        ...
                        <div
                          className="w-9 h-9 rounded-lg flex items-center justify-center text-white flex-shrink-0 md:hidden"
                        ...
                        <div className={isLeft ? 'md:text-right' : ''}>
                          <h3 className="text-sm md:text-base font-bold text-white">{step.title}</h3>
                          <p className="text-[10px] text-bridge-primary-light font-medium">{step.titleBn}</p>
                        </div>
                      </div>
                      <p className={`text-xs text-bridge-gray leading-relaxed ${isLeft ? 'md:text-right' : ''}`}>
                        {step.description}
                      </p>
                    </div>
                  </div>
