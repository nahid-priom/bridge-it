...
function FilterSection({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="mb-5">
      <h4 className="text-xs font-semibold uppercase tracking-wider text-bridge-gray mb-3">{title}</h4>
      {children}
    </div>
  );
}
...
export const SearchFilterSidebar: React.FC<SearchFilterSidebarProps> = ({
  ...
  return (
    <aside
      className={`glass-card rounded-2xl p-5 border border-white/10 ${className}`}
    >
      <div className="flex items-center justify-between mb-5">
        <h3 className="text-sm font-bold text-white">Filters</h3>
        <button
          onClick={onClearAll}
          className="text-xs text-bridge-primary-light hover:text-white transition-colors cursor-pointer"
        >
          Clear all
        </button>
      </div>
      ...
      <FilterSection title="Category">
        <select
          ...
          className="w-full px-3 py-2.5 bg-bridge-dark-2 border border-white/10 rounded-xl text-sm text-white focus:outline-none focus:border-bridge-primary cursor-pointer"
        >
      ...
      <FilterSection title="Price range (৳)">
        <div className="flex items-center gap-2">
          <input
            ...
            className="w-full px-3 py-2 bg-bridge-dark-2 border border-white/10 rounded-lg text-sm text-white focus:outline-none"
            placeholder="Min"
          />
          <span className="text-bridge-gray text-sm">–</span>
          <input
            type="number"
            min={0}
      ...
      <FilterSection title="Delivery time">
        <select
          ...
          className="w-full px-3 py-2.5 bg-bridge-dark-2 border border-white/10 rounded-xl text-sm text-white focus:outline-none cursor-pointer"
        >
          ...
          {(
            ...
            <label key={key} className="flex items-center gap-2.5 cursor-pointer group">
              <input
                ...
                className="w-4 h-4 rounded border-white/20 bg-bridge-dark-2 text-bridge-primary focus:ring-bridge-primary/30"
              />
              <span className="text-sm text-bridge-gray group-hover:text-white transition-colors">
                {label}
              </span>
            </label>
          ))}
