  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="absolute top-3 right-3 px-2 py-1 bg-bridge-dark/80 text-bridge-gray text-[10px] rounded-lg border border-white/10">
      ...
      <div className="p-4 flex flex-col flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-2">
          ...
          />
          <span className="text-xs text-bridge-gray truncate">{sellerName}</span>
          {isVerified && <Shield className="w-3.5 h-3.5 text-bridge-secondary flex-shrink-0" />}
        </div>

        <h3 className="text-sm font-bold text-white mb-1.5 line-clamp-2 group-hover:text-bridge-primary-light transition-colors">
          {title}
        </h3>
        <p className="text-xs text-bridge-gray line-clamp-2 mb-3 flex-1">{description}</p>

        <div className="flex flex-wrap items-center gap-3 mb-3 text-xs">
          <div className="flex items-center gap-1">
            <Star className="w-3.5 h-3.5 text-bridge-gold fill-bridge-gold" />
            <span className="font-bold text-white">{rating}</span>
            <span className="text-bridge-gray">({reviewCount})</span>
          </div>
        ...
        <div className="flex items-center justify-between gap-2 pt-3 border-t border-white/5 mt-auto">
          {isSeller ? (
            <span className="text-sm text-bridge-gray">View seller profile</span>
          ) : (
            <span className="text-lg font-bold text-white">
              ৳{price!.toLocaleString()}
              <span className="text-[10px] font-normal text-bridge-gray ml-1">starting</span>
            </span>
          )}
