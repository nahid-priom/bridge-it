...
import { CREATIVE_MORE_FILTERS, type CreativeMoreFilterId } from '../config/constants';

export function CreativeMoreFiltersSheet({
  applied,
  onApply,
  onClear,
}: {
  applied: CreativeMoreFilterId[];
  onApply: (next: CreativeMoreFilterId[]) => void;
  onClear: () => void;
}) {
  const [open, setOpen] = useState(false);
  const [draft, setDraft] = useState<CreativeMoreFilterId[]>(applied);
  ...
  const toggle = (id: CreativeMoreFilterId) => {
      ...
      {open ? (
        <div className="fixed inset-0 z-[80]">
          ...
          <div
            role="dialog"
            aria-modal="true"
            aria-labelledby="creative-more-filters-title"
            className={cn(
          ...
          >
            <div className="mb-4 flex items-center justify-between gap-3">
              <h2 id="creative-more-filters-title" className="font-display text-lg font-bold">
                Service type
              </h2>
              <button
                type="button"
            ...
            <div className="flex flex-wrap gap-2">
              {CREATIVE_MORE_FILTERS.map((item) => {
