...
type SoftwareFlagshipCardProps = {
  product: BitpProduct;
  topFeatures?: string[];
  className?: string;
};

export function SoftwareFlagshipCard({ product, topFeatures = [], className }: SoftwareFlagshipCardProps) {
  ...
  const bestFor = product.target_customer ?? 'Growing businesses';
  ...
  return (
    ...
    >
      <div className="relative aspect-[16/10] overflow-hidden bg-gradient-to-br from-[#0f2744] to-[#132f52]">
        <Image src={cover} alt={product.name} fill className="object-cover group-hover:scale-105 transition-transform duration-500" sizes="(max-width: 640px) 100vw, 400px" />
      ...
      <div className="flex flex-col flex-1 p-5">
        <div className="mb-3">
          {price.secondary && <p className="text-xs text-text-secondary mb-0.5">{price.secondary}</p>}
          <div className="flex items-baseline gap-2">
            <p className="text-2xl font-black text-emerald-600">{price.primary}</p>
            {price.strikethrough && <p className="text-sm text-text-secondary line-through">{price.strikethrough}</p>}
          </div>
        </div>
        ...
        <div className="flex flex-col gap-2 mt-auto">
          {demoSlug && (
            <Link href={ROUTES.softwareDemo(demoSlug)} className="deshi-btn-primary w-full inline-flex items-center justify-center gap-2 py-2.5 text-sm font-bold">
              <Play className="w-4 h-4" aria-hidden />
              Live Demo
            </Link>
          )}
          <div className="flex flex-col sm:flex-row gap-2">
            <Link href={ROUTES.softwareSolution(product.slug)} className="deshi-btn-outline flex-1 inline-flex items-center justify-center gap-2 py-2.5 text-sm font-semibold">
              View Details
              <ArrowRight className="w-4 h-4" aria-hidden />
            </Link>
            <Link href={ROUTES.softwareSolutionOrder(product.slug)} className="flex-1 inline-flex items-center justify-center gap-2 py-2.5 text-sm font-semibold rounded-xl border border-slate-200 dark:border-white/10 hover:border-emerald-500/40 transition-colors">
