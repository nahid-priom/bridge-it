...
type EcommercePreviewStripProps = {
  title: string;
  cover: string;
...
export function EcommercePreviewStrip({
  title,
  cover,
  demoSlug,
  demoUrl,
  previewUrl,
  variant,
}: EcommercePreviewStripProps) {
  const label = variant === 'admin' ? 'Admin Preview' : 'Storefront Preview';
  const href = demoSlug
    ? variant === 'admin'
      ? `${ROUTES.ecommerceDemo(demoSlug)}?view=admin`
      : ROUTES.ecommerceDemo(demoSlug)
    : previewUrl ?? demoUrl ?? null;
  ...
  return (
    <div className="rounded-2xl border border-slate-200 dark:border-white/10 overflow-hidden">
      ...
      <div className="relative aspect-video bg-gradient-to-br from-[#0f2744] to-[#132f52]">
        <Image src={cover} alt={`${title} — ${label}`} fill className="object-cover" sizes="(max-width: 768px) 100vw, 600px" />
      </div>
    </div>
  );
}
