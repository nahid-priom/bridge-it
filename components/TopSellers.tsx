            ...
            return (
              ...
              >
                ...
                <div className="relative h-28 md:h-36 overflow-hidden">
                  ...
                  {/* Category */}
                  <div className="absolute top-3 right-3 px-2.5 py-1 glass-strong rounded-lg text-[10px] font-semibold text-text-primary">
                    {cat?.icon} {cat?.name}
                  </div>
                </div>
                ...
                <div className="relative px-5 pb-5 -mt-8">
                  <div className="flex items-end gap-3 mb-3">
                    ...
                    <div className="flex-1 min-w-0 pb-1">
                      <h3 className="text-base font-bold text-text-primary truncate">{seller.name}</h3>
                      <p className="text-xs text-text-muted truncate">{seller.tagline}</p>
                    </div>
                  </div>

                  {/* Stats Row */}
                  <div className="grid grid-cols-3 gap-2 mb-3">
                    <div className="text-center p-2 bg-background-soft/60 rounded-xl border border-border-subtle">
                      <div className="flex items-center justify-center gap-1">
                        <Star className="w-3 h-3 text-bridge-gold fill-bridge-gold" />
                        <span className="text-xs font-bold text-text-primary">{seller.rating}</span>
                      </div>
                      <span className="text-[9px] text-text-muted">{seller.reviewCount} reviews</span>
                    </div>
                    <div className="text-center p-2 bg-background-soft/60 rounded-xl border border-border-subtle">
                      <span className="text-xs font-bold text-text-primary block">{seller.completedProjects}</span>
                      <span className="text-[9px] text-text-muted">Projects</span>
                    </div>
                    <div className="text-center p-2 bg-background-soft/60 rounded-xl border border-border-subtle">
                      <div className="flex items-center justify-center gap-1">
                        <Clock className="w-2.5 h-2.5 text-bridge-secondary" />
                        <span className="text-[10px] font-bold text-text-primary">{seller.responseTime}</span>
                      </div>
                      <span className="text-[9px] text-text-muted">Response</span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between pt-3 border-t border-border-subtle">
                    ...
                    <button
                      ...
                      className="flex items-center gap-1.5 text-[10px] text-bridge-primary-light hover:text-text-primary transition-colors px-2.5 py-1.5 bg-bridge-primary/10 hover:bg-bridge-primary/20 rounded-lg cursor-pointer font-medium"
                    >
